"""
Chat Service - Phase 8: Complete Chat Integration (Fixed)
...
# 导入服务（简化导入）
try:
    from llm_service_v2 import LLMService
    LLM_AVAILABLE = True
except:
    LLM_AVAILABLE = False
    print("⚠️  LLM Service 不可用")
...
try:
    ...
    print("⚠️  Prompt Service 不可用")

try:
    from hidden_event_service import HiddenEventService
    HIDDEN_EVENT_AVAILABLE = True
except:
    HIDDEN_EVENT_AVAILABLE = False
    print("⚠️  Hidden Event Service 不可用")
...
class ChatService:
    ...
    def __init__(self):
        ...
        self.memory_service = MemoryService() if MEMORY_AVAILABLE else None
        self.prompt_service = PromptService() if PROMPT_AVAILABLE else None
        self.hidden_event_service = HiddenEventService() if HIDDEN_EVENT_AVAILABLE else None
        self.inventory_service = InventoryService() if INVENTORY_AVAILABLE else None
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if self.memory_service and not self.memory_service.initialize():
                print("⚠️  Memory Service 初始化失败")
                return False

            # 初始化 Hidden Event Service
            if self.hidden_event_service and not self.hidden_event_service.initialize():
                print("⚠️  Hidden Event Service 初始化失败")
                return False

            # 初始化 Inventory Service
            if self.inventory_service and not self.inventory_service.initialize():
    ...
    async def chat(
        ...
        if self.memory_service:
            user_importance = self.prompt_service.evaluate_importance(user_message, 'user') if self.prompt_service else 0.3
        ...
        if self.hidden_event_service and character_luck >= 60:
            try:
                ...
                print(f"⚠️  隐藏事件检测错误: {e}")

        # 3. 检查道具掉落（基于角色 LCK）
        item_drop = None

        if self.inventory_service and character_luck >= 40:
            try:
                ...
                print(f"⚠️  道具掉落检测错误: {e}")

        # 4. 生成 AI 回复（如果 LLM 服务可用）
        if self.llm_service:
            # 获取相关记忆
            conversations = []
            if self.memory_service:
            ...
            if self.prompt_service:
                ...
                if self.memory_service:
                    self.memory_service.add_memory(
                        ...
                        character_id=character_id,
                        importance=assistant_importance if self.prompt_service else 0.3
                    )

        else:
            # 模拟 AI 回复（当 LLM 服务不可用时）
            demo_responses = [
                "真有意思！我很想了解更多！🌟️",
                "我理解你的感受，我也有类似体验。",
                "你说得对，这个话题很有意思！✨ 继续聊吧！"
